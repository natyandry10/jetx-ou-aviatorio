...
interface RecordedWindowPanelProps {
  records: JsonRecord[];
  sourceName?: string;
  onSearchCoefficient?: (coefficient: number) => void;
}
...
const DATE_COLOR_PALETTE = [
  { cell: 'border-sky-200 bg-sky-50 text-sky-800 dark:border-sky-800 dark:bg-sky-950/40 dark:text-sky-200', dot: 'bg-sky-500' },
  { cell: 'border-violet-200 bg-violet-50 text-violet-800 dark:border-violet-800 dark:bg-violet-950/40 dark:text-violet-200', dot: 'bg-violet-500' },
  { cell: 'border-emerald-200 bg-emerald-50 text-emerald-800 dark:border-emerald-800 dark:bg-emerald-950/40 dark:text-emerald-200', dot: 'bg-emerald-500' },
  { cell: 'border-orange-200 bg-orange-50 text-orange-800 dark:border-orange-800 dark:bg-orange-950/40 dark:text-orange-200', dot: 'bg-orange-500' },
  { cell: 'border-cyan-200 bg-cyan-50 text-cyan-800 dark:border-cyan-800 dark:bg-cyan-950/40 dark:text-cyan-200', dot: 'bg-cyan-500' },
  { cell: 'border-pink-200 bg-pink-50 text-pink-800 dark:border-pink-800 dark:bg-pink-950/40 dark:text-pink-200', dot: 'bg-pink-500' },
  { cell: 'border-lime-200 bg-lime-50 text-lime-800 dark:border-lime-800 dark:bg-lime-950/40 dark:text-lime-200', dot: 'bg-lime-500' },
  { cell: 'border-slate-300 bg-slate-100 text-slate-800 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200', dot: 'bg-slate-500' },
];

export const RecordedWindowPanel: React.FC<RecordedWindowPanelProps> = ({ records, sourceName, onSearchCoefficient }) => {
  const [now, setNow] = useState(() => new Date());
  const [dateFilter, setDateFilter] = useState<RecordedDateFilter>('all');
  const [threshold, setThreshold] = useState(15);
  const [windowMinutes, setWindowMinutes] = useState(15);
  const [precision, setPrecision] = useState(2);
  const [referenceMode, setReferenceMode] = useState<'current' | 'manual'>('current');
  ...
  useEffect(() => {
    if (referenceMode === 'manual') return undefined;
  ...
  const effectiveReference = useMemo(() => {
    if (referenceMode === 'current') return now;
    const [hours, minutes] = manualTime.split(':').map(Number);
    const reference = new Date(now);
    reference.setHours(Number.isFinite(hours) ? hours : 0, Number.isFinite(minutes) ? minutes : 0, 0, 0);
    return reference;
  }, [manualTime, now, referenceMode]);
  const dateOptions = useMemo(() => getRecordedDateOptions(records), [records]);
  const dateColorByKey = useMemo(() => new Map(dateOptions.map((option, index) => [option.value, DATE_COLOR_PALETTE[index % DATE_COLOR_PALETTE.length]])), [dateOptions]);
  const result = useMemo(() => calculateRecordedWindow(records, effectiveReference, dateFilter, threshold, windowMinutes, precision), [dateFilter, effectiveReference, precision, records, threshold, windowMinutes]);
