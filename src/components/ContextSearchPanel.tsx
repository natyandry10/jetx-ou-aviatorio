...
function relativePositionLabel(position: number): string {
  if (position === 0) return '0';
  return position > 0 ? `+${position}` : `${position}`;
}

export const ContextSearchPanel: React.FC<ContextSearchPanelProps> = ({ records, initialQuery }) => {
  ...
  const beforeItems = context?.items.filter((item) => item.position === 'before') ?? [];
  const afterItems = context?.items.filter((item) => item.position === 'after') ?? [];
  const selectedItem = context?.items.find((item) => item.position === 'selected') ?? null;
  ...
  const exportContext = () => {
    if (!contextPayload || !context) return;
