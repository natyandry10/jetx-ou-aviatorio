...
import { Archive, Clock, Download, FileDown, Play, RotateCcw, Save, Trash2 } from 'lucide-react';
...
function formatCountdown(seconds: number | null): string {
  return seconds === null ? '—' : formatDuration(Math.ceil(Math.max(0, seconds)));
}
...
function SimilarityProjectionCard({ result }: { result: SimilarityResult }) {
  ...
  const estimatedMs = result.estimatedNextTimestamp ? new Date(result.estimatedNextTimestamp).getTime() : Number.NaN;
  const secondsUntilEstimate = Number.isFinite(estimatedMs) ? (estimatedMs - nowMs) / 1000 : null;
  const estimateExpired = secondsUntilEstimate !== null && secondsUntilEstimate <= 0;
  const analysisStartMs = result.analysisStartedAt ? new Date(result.analysisStartedAt).getTime() : Number.NaN;
  const p25Timestamp = Number.isFinite(analysisStartMs) && result.delayP25Seconds !== null ? new Date(analysisStartMs + result.delayP25Seconds * 1000).toISOString() : null;
  const p75Timestamp = Number.isFinite(analysisStartMs) && result.delayP75Seconds !== null ? new Date(analysisStartMs + result.delayP75Seconds * 1000).toISOString() : null;
  useEffect(() => {
    if (!result.estimatedNextTimestamp || estimateExpired) return;
    const timer = window.setInterval(() => setNowMs(Date.now()), 1000);
    return () => window.clearInterval(timer);
  }, [result.estimatedNextTimestamp, estimateExpired]);
  const estimateState = estimateExpired ? { label: 'Échéance dépassée', className: 'bg-rose-100 text-rose-800 dark:bg-rose-950/60 dark:text-rose-200' } : { label: `En attente · dans ${formatCountdown(secondsUntilEstimate)}`, className: 'bg-emerald-100 text-emerald-800 dark:bg-emerald-950/60 dark:text-emerald-200' };
  return <div className="rounded-2xl border border-violet-300 bg-gradient-to-br from-violet-50 via-white to-indigo-50 p-4 dark:border-violet-800 dark:from-violet-950/50 dark:via-slate-900 dark:to-indigo-950/40"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-violet-700 dark:text-violet-300">Estimation du prochain tour · {routeLabel}</p><h4 className="mt-1 text-base font-bold text-slate-900 dark:text-white">Résultat calculé depuis la fenêtre LIVE sélectionnée</h4><p className="mt-1 text-[11px] text-slate-600 dark:text-slate-300">Calcul lancé à : {result.analysisStartedAt ? formatDateTime(result.analysisStartedAt) : '—'} · dernier tour sélectionné : {anchor ? formatDateTime(anchor) : '—'} · cible : &gt;{result.threshold}x · horizon : {result.horizonMinutes} min. · comparaison : {matchMode}</p></div><div className="flex flex-wrap items-center gap-2"><span className={`rounded-full px-2.5 py-1 text-[10px] font-bold ${estimateState.className}`}><Clock className="mr-1 inline-block h-3 w-3" />{estimateState.label}</span><span className={`rounded-full px-2.5 py-1 text-[10px] font-bold ${confidence.className}`}>{confidence.label}</span></div></div>{result.currentWindow.length > 0 && <div className="mt-4 rounded-xl border border-violet-200 bg-white/70 p-3 dark:border-violet-900/60 dark:bg-slate-900/60"><p className="text-[10px] font-bold uppercase tracking-wider text-violet-700 dark:text-violet-300">Chaîne sélectionnée et intervalles</p><div className="mt-2 flex flex-wrap items-center gap-2">{result.currentWindow.map((step, index) => <React.Fragment key={`${step.timestamp}-${index}`}><span className="rounded-lg border border-violet-200 bg-violet-50 px-2.5 py-1.5 text-[10px] font-mono font-bold text-violet-900 dark:border-violet-800 dark:bg-violet-950/40 dark:text-violet-100">{step.coefficient.toFixed(2)}x {index === 0 ? '(début)' : `[${formatDuration(step.intervalSeconds)}]`}</span>{index < result.currentWindow.length - 1 && <span className="text-violet-400">→</span>}</React.Fragment>)}</div><p className="mt-2 text-[10px] text-violet-800 dark:text-violet-200">Chaque intervalle est calculé entre deux multiplicateurs de la sélection triée par date et heure.</p></div>}{result.estimatedNextTimestamp && result.centralTargetCoefficient !== null ? <><div className={`mt-4 rounded-xl border p-3 ${estimateExpired ? 'border-rose-200 bg-rose-50/80 dark:border-rose-900/60 dark:bg-rose-950/30' : 'border-emerald-200 bg-emerald-50/80 dark:border-emerald-900/60 dark:bg-emerald-950/30'}`}><div className="grid grid-cols-1 gap-3 md:grid-cols-2"><div><p className="text-[10px] font-bold uppercase tracking-wider text-slate-600 dark:text-slate-300">Données observées</p><p className="mt-1 text-sm font-extrabold text-slate-900 dark:text-white">{result.currentWindow.length} résultat{result.currentWindow.length > 1 ? 's' : ''} sélectionné{result.currentWindow.length > 1 ? 's' : ''}</p><p className="mt-1 text-[10px] text-slate-600 dark:text-slate-300">Lancement : {result.analysisStartedAt ? formatDateTime(result.analysisStartedAt) : '—'}</p><p className="text-[10px] text-slate-600 dark:text-slate-300">Dernière ligne sélectionnée : {anchor ? formatDateTime(anchor) : '—'}</p></div><div className="rounded-lg border border-white/70 bg-white/70 p-2.5 dark:border-slate-800/70 dark:bg-slate-900/60"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className={`text-[10px] font-bold uppercase tracking-wider ${estimateExpired ? 'text-rose-700 dark:text-rose-300' : 'text-emerald-700 dark:text-emerald-300'}`}>{estimateExpired ? 'Échéance de l’estimation' : 'Compte à rebours de l’estimation'}</p><p className={`mt-1 text-lg font-extrabold ${estimateExpired ? 'text-rose-900 dark:text-rose-100' : 'text-emerald-900 dark:text-emerald-100'}`}>{estimateExpired ? 'Heure estimée dépassée' : `Dans ${formatCountdown(secondsUntilEstimate)}`}</p></div><p className={`text-right text-[10px] font-semibold ${estimateExpired ? 'text-rose-700 dark:text-rose-300' : 'text-emerald-700 dark:text-emerald-300'}`}>Heure calculée<br /><span className="font-mono text-sm">{formatClock(result.estimatedNextTimestamp)}</span></p></div><p className={`mt-2 text-[10px] ${estimateExpired ? 'text-rose-700 dark:text-rose-300' : 'text-emerald-700 dark:text-emerald-300'}`}>{estimateExpired ? 'Les résultats réels restent consultables dans les tableaux. Relance une analyse manuelle pour obtenir une nouvelle estimation.' : 'Le compte à rebours est informatif et ne modifie pas les données ni le calcul.'}</p></div></div><div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-1 border-t border-slate-200/70 pt-2 text-[10px] text-slate-600 dark:border-slate-700/70 dark:text-slate-300"><span><strong>Plage temporelle :</strong> {formatClock(p25Timestamp)} → {formatClock(p75Timestamp)}</span><span><strong>Référence :</strong> instant exact du lancement</span></div></div><div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4"><div className="rounded-xl border border-rose-200 bg-white/80 p-3 dark:border-rose-900/60 dark:bg-slate-900/70"><p className="text-[10px] font-bold uppercase tracking-wider text-rose-700 dark:text-rose-300">{calculationRoute === 'historical' ? 'Multiplicateur cible estimé' : 'Multiplicateur central sélection'}</p><p className="mt-1 text-2xl font-extrabold text-rose-800 dark:text-rose-200">{result.centralTargetCoefficient.toFixed(2)}x</p><p className="text-[10px] text-rose-700 dark:text-rose-300">{calculationRoute === 'historical' ? 'moyenne géométrique des cibles historiques' : 'moyenne géométrique des valeurs sélectionnées'}</p></div><div className="rounded-xl border border-indigo-200 bg-white/80 p-3 dark:border-indigo-900/60 dark:bg-slate-900/70"><p className="text-[10px] font-bold uppercase tracking-wider text-indigo-700 dark:text-indigo-300">Heure prochain tour estimée</p><p className="mt-1 text-2xl font-extrabold text-indigo-800 dark:text-indigo-200">{formatClock(result.estimatedNextTimestamp)}</p><p className="text-[10px] text-indigo-700 dark:text-indigo-300">depuis le lancement exact</p></div><div className="rounded-xl border border-cyan-200 bg-white/80 p-3 dark:border-cyan-900/60 dark:bg-slate-900/70"><p className="text-[10px] font-bold uppercase tracking-wider text-cyan-700 dark:text-cyan-300">Délai central</p><p className="mt-1 text-2xl font-extrabold text-cyan-800 dark:text-cyan-200">{formatDuration(result.centralDelaySeconds)}</p><p className="text-[10px] text-cyan-700 dark:text-cyan-300">depuis le lancement de l’analyse</p></div><div className="rounded-xl border border-amber-200 bg-white/80 p-3 dark:border-amber-900/60 dark:bg-slate-900/70"><p className="text-[10px] font-bold uppercase tracking-wider text-amber-700 dark:text-amber-300">{calculationRoute === 'historical' ? 'Cas avec cible' : `Valeurs >${result.threshold}x sélectionnées`}</p><p className="mt-1 text-2xl font-extrabold text-amber-800 dark:text-amber-200">{result.targetCount}/{targetDenominator}</p><p className="text-[10px] text-amber-700 dark:text-amber-300">{formatPercent(result.targetRate)} {calculationRoute === 'historical' ? 'dans l’horizon' : 'dans la sélection'}</p></div></div><div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-3"><div className="rounded-xl border border-slate-200 bg-white/70 p-3 dark:border-slate-800 dark:bg-slate-900/60"><p className="text-[10px] font-bold uppercase tracking-wider text-slate-600 dark:text-slate-300">Intervalle dominant</p><p className="mt-1 text-sm font-extrabold text-slate-900 dark:text-white">{result.dominantInterval?.label ?? '—'}</p><p className="text-[10px] text-slate-600 dark:text-slate-300">{result.dominantInterval ? `${result.dominantInterval.count}/${result.dominantInterval.sampleSize} intervalles` : 'aucun intervalle exploitable'}</p></div><div className="rounded-xl border border-slate-200 bg-white/70 p-3 dark:border-slate-800 dark:bg-slate-900/60"><p className="text-[10px] font-bold uppercase tracking-wider text-slate-600 dark:text-slate-300">Plage des délais P25–P75</p><p className="mt-1 text-sm font-extrabold text-slate-900 dark:text-white">{formatDuration(result.delayP25Seconds)} → {formatDuration(result.delayP75Seconds)}</p><p className="text-[10px] text-slate-600 dark:text-slate-300">min–max : {formatDuration(result.minDelaySeconds)} → {formatDuration(result.maxDelaySeconds)}</p></div><div className="rounded-xl border border-slate-200 bg-white/70 p-3 dark:border-slate-800 dark:bg-slate-900/60"><p className="text-[10px] font-bold uppercase tracking-wider text-slate-600 dark:text-slate-300">Méthodes</p><p className="mt-1 text-[10px] font-bold text-slate-900 dark:text-white">Délai : {result.delayMethod === 'trimmed-mean-20%' ? 'moyenne tronquée 20 %' : 'moyenne arithmétique (échantillon court)'}</p><p className="text-[10px] text-slate-600 dark:text-slate-300">Coefficient : moyenne géométrique</p></div></div><div className="mt-3 rounded-xl border border-violet-200 bg-white/70 p-3 text-[11px] text-violet-950 dark:border-violet-900/60 dark:bg-slate-900/60 dark:text-violet-100"><strong>Lecture prudente :</strong> l’heure affichée est calculée en ajoutant {result.delayMethod === 'trimmed-mean-20%' ? 'la moyenne tronquée des délais' : 'la moyenne arithmétique des délais (échantillon court)'} à la fin de la fenêtre sélectionnée. Le multiplicateur central utilise une moyenne géométrique, encadrée par {result.p25TargetCoefficient?.toFixed(2) ?? '—'}x → {result.p75TargetCoefficient?.toFixed(2) ?? '—'}x. L’intervalle dominant indique seulement la classe la plus fréquente dans l’échantillon. Ce n’est pas une garantie du prochain résultat.</div></> : <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50/80 p-4 text-xs text-amber-900 dark:border-amber-800 dark:bg-amber-950/30 dark:text-amber-100"><strong>Aucune estimation disponible.</strong> La sélection ne possède pas encore assez de résultats horodatés pour calculer un délai. Les nouvelles données LIVE ne recalculeront pas ce résultat automatiquement.</div>}</div>;
}
...
export const LiveAnalysisWorkbench: React.FC<LiveAnalysisWorkbenchProps> = ({ records, selectedRecords = [], onBeginSelection, onAnalysisFinished, sourceName }) => {
  ...
  const launchAnalysis = () => {
    ...
    setMessage(null);
    window.setTimeout(() => {
      const configuration = { ...selectedModel.config, sourceName: sourceName ?? 'Source LIVE', dataCount: analysisRecords.length, selectionMode: 'manual-live-grid', selectedRecordIds: analysisRecords.map((record) => record.id), analysisStartedAt };
      let result: AnalysisPayload;
      if (selectedModel.kind === 'live-similarity') {
        result = analyzeLiveSimilarity(records, selectedModel.config as Partial<Parameters<typeof analyzeLiveSimilarity>[1]>, analysisRecords, analysisStartedAt);
